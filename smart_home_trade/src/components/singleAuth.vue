...
export default{
	...
	methods:{
		              ...
		              if(that.buildId!=[]){
			             var buidlist=[]
			              for(var d=0;d<userlist.length;d++){
			              		if(userlist[d].buildingId==that.buildId[0]){
		...
		havauser(id,userlist){
			...
			that.axios.post("/SmartHomeTrade/device/selectUserIdByDeviceId",{
				...
				if(res.data.data!=null){
				  ...
				  debugger
				  that.getDepinfo(res.data.data.userIdList,userlist)
				}else{
				   that.userIdList=[]
				   that.getDepinfo([],userlist)
				}
     ...
     // 添加授权弹框
		getAuthrization(e,userlist,blockIdList){
			
			this.buildId=blockIdList;
